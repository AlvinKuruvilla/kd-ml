...
from kit import get_all_users_features_KIT, get_KIT_features_F1_from_file
from util import count_matches, pretty_print, list_avg
from plotter import make_line_plot
...
if __name__ == "__main__":
    ...
    # pretty_print(processed_selected_profile_data)
    sns.set_style("whitegrid")
    sns.kdeplot(np.array(data), bw_method=0.5)
    sns.kdeplot(np.array(processed_selected_profile_data), bw_method=0.5)
    plt.show()
    # pretty_print(selected_profile_data)
